...
use bulgur_cloud::{auth::LoginResponse, storage::{FolderResults, StorageAction}, state::PathTokenResponse};
use typescript_type_def::{
    write_definition_file,
    DefinitionFileOptions,
};

type ApiTypes = (LoginResponse, FolderResults, PathTokenResponse, StorageAction);
...
fn main() {
    ...
    if !env::current_dir().unwrap().ends_with("bulgur-cloud-frontend") {
