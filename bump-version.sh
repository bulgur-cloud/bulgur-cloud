...
sed -i -E "s/^version *= *\"([0-9.]+)\"/version = \"${NEW_VERSION}\"/" bulgur-cloud-frontend/Cargo.toml
sed -i -E "s/^( *\"version\" *: *)\"([0-9.]+)\"/\1\"${NEW_VERSION}\"/" bulgur-cloud-frontend/package.json

git add bulgur-cloud-backend/Cargo.toml bulgur-cloud-frontend/Cargo.toml bulgur-cloud-frontend/package.json
